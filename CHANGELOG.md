...
- Reject entry IDs with invalid characters in `getEntriesByIds` so that entry retrieval cannot read files outside the
  entry folder
...
- Fix commit failing when commit message contains a colon
- Fix entry retrieval failing when entries folder contains non-`.yaml` files
...
- Deprecated `checkedOutCommitHash` adapter option

## [1.0.0] - 2026-02-27

### Changed

- Upgrade to `@commitspark/git-adapter` to 1.0.0

## [0.11.0] - 2026-02-27

### Changed

- Upgrade to `@commitspark/git-adapter` to 0.30.0
